...
const MedicineList = ({ onEdit }) => {
  ...
  const [isSyncing, setIsSyncing] = useState(false); // State for the sync button loading

  // --- NEW: RESYNC FUNCTION ---
  const handleResyncAlarms = async () => {
    if (medicines.length === 0) {
        alert("No medicines to sync.");
        return;
    }

    if (!confirm(`Reset alarms for all ${medicines.length} medicines?`)) return;

    setIsSyncing(true);
    try {
        let count = 0;
        // Loop through every medicine from MongoDB
        for (const med of medicines) {
            // Force schedule the alarm on this phone
  ...
  // Allow showing list even if there's a background error (cache mode)
    ...
    <div className="space-y-4 pb-20">
      <div className="flex justify-between items-center mb-4">
          ...
          <button 
            ...
            disabled={isSyncing}
            className="text-sm bg-indigo-100 text-indigo-700 px-3 py-1 rounded-md hover:bg-indigo-200 flex items-center"
          >
        ...
        return (
          <div
            ...
            className={`bg-white rounded-lg shadow-md p-6 border-l-4 relative ${
              isOffline ? 'border-gray-400 bg-gray-50' : 
