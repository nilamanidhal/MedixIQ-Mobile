...
// @route   GET /api/admin/verify  (🚨 Replaces the old POST /login)
// @desc    Verify admin token after Firebase login and return admin data
// @access  Private (Admin Only)
router.get('/verify', authAdmin, async (req, res) => {
  try {
    // If the request made it past authAdmin, they are a verified Firebase user AND an Admin!
    req.user.lastLogin = new Date();
    await req.user.save();

    const adminData = { id: req.user.id, name: req.user.name, isAdmin: req.user.isAdmin };
